...
export default function ContactForm({
    ...
    return (
        <>
            {/* The Turnstile widget in the form pulls its script from
                challenges.cloudflare.com, and the browser only learns about
                that origin once React has mounted the widget -- it then pays
                for a DNS lookup, a TCP handshake and a TLS negotiation before
                a single byte arrives. Lighthouse measured 445ms for that on
                mobile. Warming the connection here overlaps all three with the
                rest of the page load. No crossOrigin: Turnstile's api.js is a
                plain <script src>, so a CORS-mode preconnect would open a
                second, unused connection instead of the one it needs. This
                lives in the contact section, not in the shared <head>, so
                pages without the form do not open a connection they never use. */}
            <Head>
            ...
            {/* The section keeps its .contact-details class, its pt-100 pb-100 and
                ...
                _tear-rhythm.scss measures off exactly that. The three designs that
                were not chosen live in _archive/contact-designs/ — see its README. */}
