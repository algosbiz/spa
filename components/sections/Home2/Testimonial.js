...
export default function Home2_Testimonial({
    ...
    const getSwiper = () => {
        ...
        return swiper && !swiper.destroyed && swiper.params ? swiper : null;
    };

    // Called while a quote is clamped, so the slider height matches the toggle we just revealed.
    const handleMeasure = useCallback(() => {
        getSwiper()?.updateAutoHeight(0);
    }, []);

    // Reading a full review takes longer than the 5s autoplay beat, so opening one stops it.
