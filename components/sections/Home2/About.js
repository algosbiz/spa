...
export default function Home2_About({
    ...
    return (
        ...
        {showBrandCard && (
            <style jsx global>{`
                /* .brand-card and .circle-btn are both pinned to right:0 bottom:0
                   of the same column, so they stack. When the brand card is on,
                   hide the arrow button and let the card sit exactly on the gold
                   circle instead of floating inside it as a smaller square. */
                .about-section-two .image-column .inner-column .circle-btn--replaced {
                    display: none;
                }
