...
// Slug + last-changed date + cover image for every published post
// (for sitemap.xml, whose <image:image> block needs the cover).
export async function getPublishedSlugsWithDates() {
    const { data, error } = await supabaseAdmin
        .from('posts')
