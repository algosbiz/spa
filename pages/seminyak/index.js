...
// Category split mirrors the outcall price list so the same treatment always
// lands in the same tab on both pages.
const BEAUTY_TREATMENT_NAMES = new Set([
...
const COUPLE_TREATMENT_NAME = "Couple Massage";

const massageTreatments = pricingTreatments.filter(
  (treatment) =>
    treatment.name !== COUPLE_TREATMENT_NAME &&
    !BEAUTY_TREATMENT_NAMES.has(treatment.name)
);

const beautyTreatments = pricingTreatments.filter((treatment) =>
  BEAUTY_TREATMENT_NAMES.has(treatment.name)
);

// The couple entry is a single parent holding every couple option. The tab reads
// better with each option as its own row, the way the outcall page lists them.
const coupleTreatments = (
  pricingTreatments.find((treatment) => treatment.name === COUPLE_TREATMENT_NAME)
    ?.children || []
).map((child, index) => ({
  ...child,
  id: `couple-${index + 1}`,
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        /* The pill tabs live under .outcall-page in the stylesheet. Mirror them
           here so the category tabs read the same on the price list. */
        .pricing-package-section .package-tab ul.nav.nav-tabs {
          gap: 16px;
        }
        ...
        .pricing-package-section
          ...
          outline: 3px solid rgba(167, 134, 39, 0.3);
          outline-offset: 3px;
        }
